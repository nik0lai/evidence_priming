...
# mask
participant_label_mask <- make_participant_label(mask_dat)
...
prime_dat_summ <-
  ...
  full_join(
    bf_data_prime %>%
      select(participant, soa, bf, bf_number, bf_pretty, label, bf_label, cohensd, d_label) %>% 
      make_soa_factor()
  )
...
p_prime <-
  ...
  stat_unique(aes(x=as.factor(soa), y=y_bf-.1, label=bf_number, color=bf_label), size=3, parse=TRUE, geom='text') +
...
mask_priming <- 
  ...
  full_join(
    bf_data_mask %>%
      select(participant, soa, bf, bf_number, bf_pretty, label, bf_label, d_label) %>% 
      make_soa_factor()
  )
...
p_mask <-
  ...
  stat_unique(aes(x=as.factor(soa), y=y_bf, label="bold(BF[10])", color=bf_label), size=3, parse=TRUE, geom='text') +
  stat_unique(aes(x=as.factor(soa), y=y_bf-18, label=bf_number, color=bf_label), size=3, parse=TRUE, geom='text') +
...
(p_prime + plot_spacer() + p_mask) + 
  plot_layout(widths = c(4, -.48, 4), guides = 'collect') & 
  theme(legend.position = 'bottom')

ggsave('plots/fig4_train_subjects.png', width = 12, height = 18,  dpi = 600, scale=.6, device = png)

# Make markdown tables for paper ------------------------------------------

# Build wide-format table with separate BF and d rows
table_md <- build_participant_bf_table(bf_data_prime)
write_clip(table_md)   # now it's in your clipboard

# Masking -----------------------------------------------------------------

# Build wide-format table with separate BF and d rows
table_md <- build_participant_bf_table(bf_data_mask)
write_clip(table_md)   # now it's in your clipboard
