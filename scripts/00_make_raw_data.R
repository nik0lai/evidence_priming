# This script is only used to create the public raw-data files included in this repository.
# It depends on source files that are not distributed in the GitHub or OSF copies,
# so external users should not expect this script to run as-is.
...
dat_control <- 
  dat_control %>% 
  select(participant, block_count, trial_count, trial_aborted, soa, congruent, task, prime_presence, 
         prime_direction, mask_direction, stim_position, mask_answer, mask_rt, prime_answer, prime_rt, mask_accuracy, prime_accuracy)

dat_train <- dat_train %>% 
  select(participant, session, trial_count, trial_aborted, soa, congruent, task, 
         prime_direction, mask_direction, stim_position, answer, rt, accuracy)
