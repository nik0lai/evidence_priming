...
p_load(magrittr, readr, dplyr, tidyr, ggplot2, tibble, stringr, purrr, furrr, patchwork, glue)
...
format_result <- function(data, soa_value) {
  row <- data %>% filter(soa == soa_value)
  
  glue(
    "{soa_ms(row$soa)} ms: {pretty_bf(row$bf)}, *d* = {pretty_d(row$cohensd)}"
  )
}

paper_paragraph <- glue(
  "When participants had to discriminate the direction of the prime, there was anecdotal to very strong evidence for chance performance at the three shortest SOAs ({format_result(bf_prime, 0.0125)}; {format_result(bf_prime, 0.025)}; {format_result(bf_prime, 0.0375)}), whereas there was moderate to extreme evidence for above-chance performance at the three longest SOAs ({format_result(bf_prime, 0.05)}; {format_result(bf_prime, 0.0625)}; {format_result(bf_prime, 0.075)}; see Figure [-@fig:fig2-controlExp]A). Note, however, that even at the longest SOA, the effect size did not exceed {pretty_d(max(bf_prime$cohensd))}, suggesting that participants performed barely above chance. When discriminating the direction of the mask, except for the shortest SOA ({format_result(bf_mask, 0.0125)}), participants showed reliable priming effects, i.e. they were faster on congruent compared to incongruent trials ({format_result(bf_mask, 0.025)}; {format_result(bf_mask, 0.0375)}; {format_result(bf_mask, 0.05)}; {format_result(bf_mask, 0.0625)}; {format_result(bf_mask, 0.075)}; see Figure [-@fig:fig2-controlExp]B)"
)
