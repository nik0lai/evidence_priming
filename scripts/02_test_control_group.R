...
get_alt_fit <- function(df) {
  ...
  filename <- sprintf('model_fits/control_group/fit_control_prime_group_%s', unique(df$soa))
  file_rds <- paste0(filename, '.rds')

  if (file.exists(file_rds)) {
    model_alt <- readRDS(file_rds)
  } else {
    model_alt <- update(object = comp_prime, newdata = df, recompile = FALSE, chains = 4, iter = 5000, cores = 4, file = filename)
  }
...
for (r in 1:nrow(prime_dat_disc)) {
  ...
  ggsave(plot_name, plot = p[[1]], width = 7, height = 8, bg='white')
}
...
get_alt_fit_mask <- function(df) {
  ...
  filename <- sprintf('model_fits/control_group/fit_control_mask_group_%s', unique(df$soa))
  file_rds <- paste0(filename, '.rds')

  if (file.exists(file_rds)) {
    model_alt <- readRDS(file_rds)
  } else {
    model_alt <- update(object = comp_alt_mask, newdata = df, recompile = FALSE, chains = 4, iter = 5000, cores = 4, file = filename)
  }
