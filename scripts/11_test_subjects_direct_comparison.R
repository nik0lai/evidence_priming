...
sample_model <- function(df) {
  ...
  filename <- sprintf('model_fits/direct_comparison_train_subjects/fit_direct_subjects_%s_%s', unique(df$participant), unique(df$soa))
  file_rds <- paste0(filename, '.rds')

  if (file.exists(file_rds)) {
    model_alt <- readRDS(file_rds)
  } else {
    model_alt <- update(object = m, newdata = df, recompile = FALSE, chains = 4, iter = 10000, cores = 4, file = filename)
  }
