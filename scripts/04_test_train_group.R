...
# alternative model
comp_prime <- brm(
  y_behav ~ x_stim + session + (1 | participant),
...
get_alt_fit <- function(df) {
  ...
  filename <- sprintf('model_fits/train_group/fit_train_prime_group_%s', unique(df$soa))
  file_rds <- paste0(filename, '.rds')

  if (file.exists(file_rds)) {
    model_alt <- readRDS(file_rds)
  } else {
    model_alt <- update(object = comp_prime, newdata = df, recompile = FALSE, chains = 4, iter = 6000, cores = 4, file = filename)
  }
...
for (r in 1:nrow(prime_data)) {
  ...
  p <- brms::mcmc_plot(prime_data[[r, 'bf_alt']][[1]], variable='^b_|^sd_', regex = TRUE, type='trace')
  ggsave(plot_name, plot = p, width = 7, height = 8, bg='white')
}
...
get_alt_fit_mask <- function(df) {
  ...
  filename <- sprintf('model_fits/train_group/fit_train_mask_group_%s', unique(df$soa))
  file_rds <- paste0(filename, '.rds')

  if (file.exists(file_rds)) {
    model_alt <- readRDS(file_rds)
  } else {
    model_alt <- update(object = comp_alt_mask, newdata = df, recompile = FALSE, chains = 4, iter = 6000, cores = 4, file = filename)
  }
...
for (r in 1:nrow(mask_data)) {
  ...
  p <- brms::mcmc_plot(mask_data[[r, 'bf_alt']][[1]], variable='^b_|^sd_', regex = TRUE, type='trace')
  ggsave(plot_name, plot = p, width = 7, height = 8, bg='white')
}
# Save data ---------------------------------------------------------------
