...
get_alt_fit_mask <- function(df) {
  ...
  filename <- sprintf('model_fits/trimming_test/fit_train_mask_subjects_%s_%s_%s', unique(df$participant), unique(df$soa), unique(df$trim))
...
train_mask_subjects_fits %>% 
  ...
  write_csv('results/train_mask_subjects_trim_bf.csv')

# Compare BF and effect size ----------------------------------------------

# read BF and cohens' d 
train_mask_subjects_bf_d <- 
  read_csv('results/train_mask_subjects_bf.csv') %>% 
  select(-sessions)

train_mask_subjects_fits %>% 
  select(-c(cohensd, data, bf_alt)) %>%
  bind_rows(train_mask_subjects_bf_d %>% select(-cohensd) %>% mutate(trim = '2.3sd')) %>% 
  mutate(bf = log(bf)) %>% 
...
train_mask_subjects_fits %>%
  ...
  bind_rows(train_mask_subjects_bf_d %>% select(-bf) %>% mutate(trim = '2.3sd')) %>%
