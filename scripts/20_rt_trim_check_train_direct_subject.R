...
dir_comp_data_2sd <-
  dir_comp_data_2sd %>% 
  group_by(participant, soa, trim) %>% 
  nest() %>% 
  mutate(data = map(data, ~.x %>% mutate(participant = participant, soa = soa, trim = trim)))
dir_comp_data_2.5sd <-
  dir_comp_data_2.5sd %>% 
  group_by(participant, soa, trim) %>% 
  nest() %>% 
...
sample_model <- function(df) {
  ...
  filename <- sprintf('model_fits/trimming_test/fit_direct_subjects_%s_%s_%s', unique(df$participant), unique(df$soa), unique(df$trim))
...
dir_comp_subject_fits %>% 
  ...
  arrange(trim, participant, soa) %>% 
  write_csv('results/train_direct_comparison_subjects_trim_bf.csv')

# Compare BF and effect size ----------------------------------------------

# read BF and cohens' d 
train_direct_comp_group_subjects_bf_d <- 
  read_csv('results/train_direct_comparison_bf.csv') 

dir_comp_subject_fits %>% 
  select(-c(cohensd, data, bf_alt)) %>%
  bind_rows(train_direct_comp_group_subjects_bf_d %>% select(-cohensd) %>% mutate(trim = '2.3sd')) %>% 
  mutate(bf = log(bf)) %>% 
...
dir_comp_subject_fits %>%
  ...
  bind_rows(train_direct_comp_group_subjects_bf_d %>% select(-bf) %>% mutate(trim = '2.3sd')) %>%
...
comp %>%
  arrange(participant, trim, stat, soa) %>% 
  make_trim_table_subject() %>% 
  write_clip()
